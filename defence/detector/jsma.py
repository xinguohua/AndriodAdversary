...
if __name__ == "__main__":
    ...
    trained_model = tf.keras.models.load_model('D:\\安全课程\\android detection\\Android-Malware-Detection-Adversarial-Examples-master\\feature_based_original_dataset\\best_model_Adam.h5')
    # incremental learned model
    #trained_model = tf.keras.models.load_model('models_incremental_learning/model_Adam.h5')
    # adversarial trained model
    #trained_model = tf.keras.models.load_model('Adam_adversarial_training_adv_800_0.3.h5')
    # distilled model
    #trained_model = tf.keras.models.load_model('defensive_distillation/distilled-100', custom_objects={'fn': fn})


    averageChanges = 0

    # val_data = pd.read_csv( 'D:\\安全课程\\android detection\\Android-Malware-Detection-Adversarial-Examples-master\\drebin\\x_test.csv')
    # val_labels = pd.read_csv( 'D:\\安全课程\\android detection\\Android-Malware-Detection-Adversarial-Examples-master\\drebin\\y_test.csv')
    ...
    #0，1型数据
    val_data = np.loadtxt(open("D:\\data\\x_test01.csv", "rb"), delimiter=",", skiprows=0,dtype=np.int32)
    val_labels = np.loadtxt(open("D:\\data\\y_test01.csv", "rb"), delimiter=",", skiprows=0,dtype=np.int32)
    # print(val_labels)
    ...
    predictions = trained_model.predict(X_adv)
    confusion = confusion_matrix(Y_adv, np.argmax(predictions, axis=1))
    print(confusion)
    TP = confusion[1, 1]
    TN = confusion[0, 0]
    FP = confusion[0, 1]
    ...
    '''adv_trained_model = tf.keras.models.load_model("Adam_adversarial_training_adv_100_0.7.h5")
    predictions = adv_trained_model.predict(val_data)
    confusion = confusion_matrix(val_labels, np.argmax(predictions, axis=1))
    print(confusion)
    TP = confusion[1, 1]
    TN = confusion[0, 0]
    ...
    # evaluate adversarial examples produced from one model to other ML models
    #evaluate_other_models()
