...
def get_kd(model, X_train, Y_train, X_test, X_test_noisy, X_test_adv):
    ...
    """
    # Get deep feature representations 得到神经网络最深层输出 样本个数*128维
...
def main(args):
    ...
    if args.attack == 'all':
        ...
        # Load adversarial samples 加载对抗样本
    ...
    # Check model accuracies on each sample type 评价三种类型样本的准确性
    ...
    if args.characteristic == 'kd':
        ...
        file_name = os.path.join(PATH_DATA, 'lid_%s_%s_%s.npy' %
                                 (args.dataset, args.attack, args.k_nearest))
        data = np.concatenate((characteristics, labels), axis=1)
        np.save(file_name, data)
