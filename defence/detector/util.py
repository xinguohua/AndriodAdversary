...
def get_model(dataset='mnist', softmax=True):
    ...
    if dataset == 'mnist':
        ...
        layers = [
            ...
            Dense(128),  # 9            
            Activation('relu'),  # 10
            normalization.BatchNormalization(), # 11
            Dropout(0.5),  # 12
            Dense(10),  # 13
        ]
    elif dataset == 'cifar':
        ...
        layers = [
            ...
            Activation('relu'),  # 8
            normalization.BatchNormalization(), # 9
            Conv2D(64, (3, 3), padding='same'),  # 10
            Activation('relu'),  # 11
            normalization.BatchNormalization(), # 12
            MaxPooling2D(pool_size=(2, 2)),  # 13
            
            Conv2D(128, (3, 3), padding='same'),  # 14
            Activation('relu'),  # 15
            normalization.BatchNormalization(), # 16
            Conv2D(128, (3, 3), padding='same'),  # 17
            Activation('relu'),  # 18
            normalization.BatchNormalization(), # 19
            ...
            Activation('relu'),  # 24
            normalization.BatchNormalization(), # 25
            Dropout(0.5),  # 26
            Dense(512, kernel_regularizer=l2(0.01), bias_regularizer=l2(0.01)),  # 27
            Activation('relu'),  # 28
            normalization.BatchNormalization(), # 29
            Dropout(0.5),  # 30
            Dense(10),  # 31
        ]
    else:
        ...
        layers = [
            ...
            Dense(128),  # 13
            Activation('relu'),  # 14
            normalization.BatchNormalization(), # 15
            Dropout(0.5),  # 16
            Dense(10),  # 17
        ]
...
def get_mc_predictions(model, X, nb_iter=50, batch_size=256):
    ...
    :return:
    """
    output_dim = model.layers[-1].output.shape[-1].value #输出维度
    ...
    def predict():
        n_batches = int(np.ceil(X.shape[0] / float(batch_size)))  #处理几次批次
        output = np.zeros(shape=(len(X), output_dim)) #样本个数*输出维度10
...
def get_deep_representations(model, X, batch_size=256):
    ...
    get_encoding = K.function(
        [model.layers[0].input, K.learning_phase()],
        [model.layers[3].output] #最后一个隐藏层的输出
    )
    ###处理几次批次 np.ceil向上取整 X样本个数/batch大小
    n_batches = int(np.ceil(X.shape[0] / float(batch_size)))
    output = np.zeros(shape=(len(X), output_dim)) ##输出一个X样本个数*输出维度的矩阵
    for i in range(n_batches):
        output[i * batch_size:(i + 1) * batch_size] = \
...
def get_layer_wise_activations(model):
    """
    Get the deep activation outputs. 获取深度激活输出。
    :param model:
    :return: 
    """
...
def get_lids_random_batch(model, X, X_adv,k=10, batch_size=100):
    """
    ...
    # func是一个函数组 输入是初始层的输入 输出是每一层经过激活函数后的输出
    funcs = [K.function([model.layers[0].input, K.learning_phase()], [out])
                 for out in get_layer_wise_activations(model)]
    lid_dim = len(funcs) #层数
    print("Number of layers to estimate: ", lid_dim)

    def estimate(i_batch):
        start = i_batch * batch_size  #开始的位置
        end = np.minimum(len(X), (i_batch + 1) * batch_size)  #结束的位置
        n_feed = end - start
        lid_batch = np.zeros(shape=(n_feed, lid_dim))  #lid 是一个批次个数*层数的形状
        lid_batch_adv = np.zeros(shape=(n_feed, lid_dim))
        for i, func in enumerate(funcs): #feed个样本依次经过所有的层，每一层得到激活后的值X_act
    ...
    for i_batch in tqdm(range(n_batches)):
        lid_batch, lid_batch_adv = estimate(i_batch)  #一批的个数*每层的lid值
        lids.extend(lid_batch)  #每次增加100个
...
def score_samples(kdes, samples, preds, n_jobs=None):
    ...
    results = np.asarray(
        p.map(
            score_point, #列表中的值: 循环test样本和test样本对应的标签值   得到(test样本,标签对应的和密度估计函数)
            [(x, kdes[i]) for x, i in zip(samples, preds)] #列表中的值: 循环test样本和test样本对应的标签值   得到(test样本,标签对应的和密度估计函数)
        )
    )
    p.close()
    p.join()
