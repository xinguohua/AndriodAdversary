...
def extractAdvUncertity(args):
    # 攻击参数在的范围
    assert args.attack in ['jsmf', 'deepfool', 'onefeature', 'fgsm'], \
        "Attack parameter must be either 'jsmf', 'deepfool', 'onefeature', 'fgsm'"

    # 对抗样本+参数里攻击路径
    MareWareTestset_from_csv = MareWareDataset(PATH_DATA+args.attack+'//'+args.attack+'_200_200_X_adv.csv', PATH_DATA+args.attack+'//'+args.attack+'_200_200_Y_adv.csv', transformations)
    ...
    print('adv data len: ', len(test_loader.dataset))
    ...
    for batch_idx, (test_x, test_y) in enumerate(test_loader):
        ...
        for id, z in enumerate(prob_avearray):
            ...
            # varmean = np.abs(x)[0]  # 一个向量各个值代表方差平均  取良性分类的方差
            # varmean = np.abs(x).mean() #一个向量取平均
            # print(varmean)
...
def extractNormalUncertity(args):
    # 攻击参数在的范围
    assert args.attack in ['jsmf', 'deepfool', 'onefeature', 'fgsm'], \
        "Attack parameter must be either 'jsmf', 'deepfool', 'onefeature', 'fgsm'"
...
if __name__ == "__main__":
    ...
    #提取对抗样本不确定性到csv
    extractAdvUncertity(args)

    # 提取正常样本不确定性到csv
    extractNormalUncertity(args)
