...
正常和对抗计算误分类率(两个FNR相减)
## evaluate_second_adv.py(评价第二种模式攻击)
选定一种分类器（加载一个特定的模型） 看不同攻击的效果（不同数据集）
...
运行Analyze/evaluate_adv.py(**此时用的数据是jsmf3000sample--大样本**)
* 对于同一模型加载不同对抗样本 得到一个表的数据
* 不同架构DNN重复上述操作 换加载模型 得到所有表的数据


## 3 第二种攻击模式重复上述操作（记录平均扰动，evaluate_second_adv.py）
**此时用的数据是jsmf3000sample--大样本**
## deepfool
tutorials/deepfoolattackall_second.py 生成第二种攻击模型下的对抗样本
## fgsmattak
tutorials/fgsmattackall_second.py 生成第二种攻击模型下的对抗样本
...
* 将得到的jsmf对抗样本分别与训练数据混合训练
	* 分别以比例advdata*0.08 30（实际混入29）加入对抗样本 得到模型 adversarial_jsmf30_model.h5
...
共两类模型 adversarial_jsmfxx_model.h5 adversarial_jsmfxx_model.h5
...
## 11 训练&&评价蒸馏模型
运行malwareclassification/defensive_distillation.py在T=150情况下

训练蒸馏模型保存到models/distillation_model.h5

运行Analyze/evaluate_distillation.py
