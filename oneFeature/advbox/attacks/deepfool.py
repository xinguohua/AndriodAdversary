    ...
    def _apply(self, adversary, iterations=100, overshoot=0.02):
        ...
        for iteration in range(iterations): #每次迭代相当于修改一个特征
            ...
            if adversary.try_accept_the_example(maty, adv_label):
                ...
                save_name = str(iteration+1)+"featuresbestSA" + "_" + "_sa_x_fitnees_pic"
                # 迭代次数和fitness图
                print(len(allX))
                print(len(allY))
                ut.plotgraph(allX, allY,
                             str(iteration+1)+"featuresbestSA" + "_" + "_sa_x_fitnees_pic",
                             save_name)
                for i in range(len(allX)):
                    ut.plotsinglegraph(allX[i], allY[i],
                                       str(iteration + 1) + "featuresbestvalues" + "_" + "_sa_x_fitnees_pic",
                                       str(i))
                save_name1 = str(iteration + 1) + "featuresbestvalues" + "_" + "_sa_x_fitnees_pic"
                print(valuesx)
                print(valuesy)
                # 画图
