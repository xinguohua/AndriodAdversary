import random
from secondattack.deepfool.sa import AnnealingFunctions
import time
...
class SimulatedAnnealing(Algorithm):
    def __init__(self,max_iterations,temp_max,temp_change,KnapsackObj,doindex):
    ...
    def run(self,templist):
        start_time = int(round(time.time() * 1000))

        # 得到初始状态解决方案
        viewindex=[]
        current,index = self.AF.getInitSolution(templist,self.doindex)
        ...
        for i in range(self.MaxIterations):
            ...
            neigbourSolution,index = self.AF.getNeighbouringSolution(templist,viewindex,self.doindex)
            viewindex.append(index)
