import random
from secondattack.oneFeature.sa import AnnealingFunctions
import time
...
class SimulatedAnnealing(Algorithm):
    ...
    def run(self,templist):
        ...
        # 得到初始状态解决方案
        viewindex=[]
        current,index = self.AF.getInitSolution(templist)
        ...
        for i in range(self.MaxIterations): #模拟退火内层迭代
            #print("模拟退火内层"+str(i)+"次迭代")

            neigbourSolution,index = self.AF.getNeighbouringSolution(templist,viewindex)
            viewindex.append(index)
        ...
        #内部迭代过程
        #最好方案 内部迭代坐标  内部迭代对应的fiiness值 操作 时间
