...
if __name__ == "__main__":
    ...
    #加载测试数据 小数据
    # val_data = np.loadtxt(open("..//..//data//X_train.csv", "rb"), delimiter=",", skiprows=0, dtype=np.int32)
    # val_labels = np.loadtxt(open("..//..//data//Y_train.csv", "rb"), delimiter=",", skiprows=0, dtype=np.int32)
    # print(val_labels)
    ...
    #对抗样本
    np.savetxt('jsmf_X_adv.csv', X_adv, delimiter=',')
    np.savetxt('jsmf_Y_adv.csv', Y_adv, delimiter=',')
    print("保存成功")
